...
# Perform a linear regression anlysis on all potentially differentially expressed genes to determine
# which MDV VIRAL GENES are most associated with IKZF1 expression levels

# Transfer the gene counts from the rna gene expression of the MDV genome
rsync -avp steepale@rsync.hpcc.msu.edu:/mnt/research/ADOL/OutsideCollaborations/20160201_Cheng_Steep_Xu_Zhang/RNA_DE/data/gene_counts_mdv/*_gene_counts.txt \
./data/gene_counts_mdv/

# Generate the R command to create a vector of sample names chicken genome
(echo 'samples <- c'; \
find ./data/gene_counts_mdv -name *_gene_counts.txt | \
xargs basename | \
sed 's/_ensembl_gene_counts.txt//' | \
sort | uniq | \
sed '$ ! s/.*/"&",/; $ s/.*/"&")/' | \
tr '\n' ' ') | tr '\n' '('
# samples <- c("017738-1", "017741-1", "017756-3", "017766-1", "017777-3", "017787-2", "017794-1", "017798-1_1", "017798-1_2", "017833-1", "017834-2", "017835-1", "017841-3", "017842-2_1", "017842-2_2", "017855-1_1", "017855-1_2", "017863-1", "017884-2", "017901-2_1", "017901-2_2", "017906-1", "017911-1_1", "017911-1_2", "017918-3", "017927-2")

mkdir ./analysis/plots/lm_mrna_counts_mdv/

# IKZF1 Counts

# ./scripts/mdv_gene_expression_vs_ikzf1_linear_regression.R
##########################################
